...
export async function searchConversations(query, options = {}) {
    ...
    return results.map((row) => {
        ...
        return {
            exchange,
            similarity: mode === 'text' ? undefined : Math.min(1, Math.max(0, 1 - row.distance)),
            snippet,
            summary
        };
    });
...
export async function searchMultipleConcepts(concepts, options = {}) {
    ...
    const conceptResults = await Promise.all(concepts.map(concept => searchConversations(concept, { ...options, limit: limit * 5, mode: 'both' })));
